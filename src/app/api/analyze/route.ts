...
- 画像内に栄養成分表示（カロリー、タンパク質、脂質、炭水化物）や内容量（g）の記載が読み取れる場合は、その数値を最優先で抽出し、\`label_nutrition\` に含めてください。
- 市販品やコンビニ商品と思われる場合は、メーカー名やブランド名を含めた具体的な商品名（例：『セブンプレミアム サラダチキン ハーブ』）を name に設定してください。
...
- コンビニおにぎりの場合:
  {
    "foods": [{ 
      "name": "セブンイレブン 手巻おにぎり 紀州南高梅", 
      "amount": "1個",
      "label_nutrition": { "calories": 171, "protein": 3.3, "fat": 1.2, "carbs": 38.2 }
    }],
    "is_ambiguous": false
  }
...
{ 
  "foods": [{ 
    "name": "食品名（日本語）", 
    "amount": "量の目安",
    "label_nutrition": { "calories": 数値, "protein": 数値, "fat": 数値, "carbs": 数値 } // ラベルから読み取れた場合のみ
  }],
...
function parseFoodListJson(content: string): { foods: { name: string; amount?: string; label_nutrition?: any }[], is_ambiguous?: boolean, reason?: string } {
  ...
  try {
    ...
    const foods = arr.map((item: any) => {
      ...
      return { 
        ...
        amount: item.amount,
        label_nutrition: item.label_nutrition
      };
    });
...
export async function recognizeWithOpenAI(base64Image: string): Promise<{ foods: { name: string; amount?: string; label_nutrition?: any }[], is_ambiguous?: boolean, reason?: string }> {
...
export async function recognizeWithGemini(base64Image: string): Promise<{ foods: { name: string; amount?: string; label_nutrition?: any }[], is_ambiguous?: boolean, reason?: string }> {
  const apiKey = process.env.GEMINI_API_KEY ?? "";
  const endpoint = `https://generativelanguage.googleapis.com/v1alpha/models/gemini-3.1-flash-lite:generateContent?key=${apiKey}`;
