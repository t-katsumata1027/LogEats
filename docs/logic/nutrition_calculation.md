...
- **【NEW】ラベル優先抽出**: パッケージの「栄養成分表示」が明確に写っている場合、その数値を `label_nutrition` として直接抽出します。
- **出力**:
  - `foods` 配列（`name`: 食品名, `amount`: 例 "茶碗1杯(約150g)", `label_nutrition`: オプション）
  - `is_ambiguous` (不鮮明、食事以外のフラグ)
- **現状の精度向上ポイント**: 
  - コンビニ等の市販品において、AIの推論を介さず「パッケージの裏面」を読み取らせることで、100%正確な公式データを取得できるようになりました。
  - プロンプトレベルでブランド名を含む正確な名称を抽出させ、3,500件超のマスターDBと照合します。
